...
<% 
    ...
    ssql="Select kisiler.id,kisiler.foto_path,kullanicilar.ad,kullanicilar.soyad from kullanicilar inner join kisiler on kullanicilar.id=kisiler.kullanici_id;"
...
<head>
    ...
    <title>Mezunlar Sitesi || Kişiler </title>
</head>
<body>
    <header></header>
        <nav>
            <ul class="menu1" nav="yukari">
        ...
        <section>
            <div class="kisiler">
                <table class="kisiler-table">
                    <tr>
                        <th width="30">#</th>
                        <th width="50">Kişiler</th>
                    </tr>
                    <% 
                    Do While NOT rs.EOF
                        Response.write("<tr>")
                    %>
                        <td>
                           <a href="kisi.asp?id=<%=rs("id")%>" target="_self">
                                <img src="<%=rs("foto_path")%>" alt="<%=rs("ad")%> <%=rs("soyad")%>">
                           </a> 
                        </td>
                        <td><a href="kisi.asp?id=<%=rs("id")%>" target="_self"><%=rs("ad")%>&nbsp<%=rs("soyad")%></a></td>
